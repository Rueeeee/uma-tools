...
import { aggregateActivityRewards, dateRangeTimestamps, eventsOverlap } from "./intelActivityAggregate";
...
function EventFilters({
  ...
  onTypeChange,
  onQueryChange,
  startDate, endDate, onStartDateChange, onEndDateChange, onClearDates,
}: {
  type: string; query: string; types: string[];
  onTypeChange: (value: string) => void; onQueryChange: (value: string) => void;
  startDate: string; endDate: string; onStartDateChange: (v:string)=>void; onEndDateChange:(v:string)=>void; onClearDates:()=>void;
}) {
  return (
    <div class="intelFilterBar">
      <select
      ...
      <input value={query} onInput={(event) => onQueryChange(event.currentTarget.value)} type="search" placeholder="搜索活动" aria-label="搜索活动" />
      <input type="date" value={startDate} onInput={(e)=>onStartDateChange(e.currentTarget.value)} aria-label="开始日期（北京时间）" />
      <input type="date" value={endDate} onInput={(e)=>onEndDateChange(e.currentTarget.value)} aria-label="结束日期（北京时间）" />
      {(startDate || endDate) && <button type="button" onClick={onClearDates}>清除日期</button>}
    </div>
  );
}
...
export function IntelDashboard() {
  ...
  const filteredEvents = useMemo(() => {
    ...
    const range = eventStartDate && eventEndDate
      ? dateRangeTimestamps(eventStartDate, eventEndDate)
      : null;
    const sourceEvents = range ? (data.events || []) : events;
    return sourceEvents
      .slice()
      ...
      .filter((event) => {
        ...
        if (eventStartDate || eventEndDate) {
          if (!range || !eventsOverlap(event, range)) return false;
        }
        return true;
      });
  }, [events, eventTypeFilter, eventQuery, eventStartDate, eventEndDate]);
  const eventRewardSummary = useMemo(
    () => aggregateActivityRewards(filteredEvents),
    [filteredEvents],
  );
    ...
    <main class="intelPage">
      ...
      {activeTab === "events" && (
        <section class="intelCalendarPanel">
          <div class="intelSectionTitle">
            <div class="intelSectionActions">
              <EventFilters
                ...
                onStartDateChange={setEventStartDate} onEndDateChange={setEventEndDate}
                onClearDates={() => { setEventStartDate(""); setEventEndDate(""); }}
              />
          ...
          {eventStartDate && eventEndDate && dateRangeTimestamps(eventStartDate, eventEndDate) && (
            <div class="intelRewardSummary" aria-label="活动奖励汇总">
              <strong>活动奖励汇总</strong>
              <small>统计与区间重叠活动的整场已知奖励，不按天折算。</small>
              {eventRewardSummary.length ? (
                <table class="intelRewardSummaryTable">
                  <thead><tr><th scope="col">奖励</th><th scope="col">数量</th></tr></thead>
                  <tbody>{eventRewardSummary.map((reward) => (
                    <tr key={reward.key}>
                      <td>{reward.image && <img src={assetUrl(reward.image)} alt="" />}<span title={reward.name}>{reward.name}</span></td>
                      <td class="intelRewardQuantity">{reward.unknown ? (reward.amount > 0 ? `≥${reward.amount}（部分待确认）` : "数量待确认") : `×${reward.amount}`}</td>
                    </tr>
                  ))}</tbody>
                </table>
              ) : <span>所选区间没有可汇总的已知奖励</span>}
            </div>
          )}
          {eventView === "list" ? (
